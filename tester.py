...
class Tester(object):
    ...
    def test(self):
        ...
        test_save_path = os.path.join(self.test_result_path, 'test_results')
        test_compare_save_path = os.path.join(self.test_result_path, 'test_compare')
        ...
        os.makedirs(test_compare_save_path, exist_ok=True)
        ...
        with torch.no_grad():
            for test_step in range(test_start, test_total_steps):
                ...
                for i in range(0, denorm(test_real_raw.data).size(0)):
                    ...
                    save_imgs = denorm(test_fake_exp.data)[i:i + 1, :, :, :]
                    save_path = os.path.join(test_save_path, '{:s}_{:0>3.2f}_testFakeExp.png'.format(test_name[i], self.args.pretrained_model))
                    os.makedirs(os.path.dirname(save_path), exist_ok=True)
                    save_image(save_imgs, save_path)

                    save_imgs_compare = torch.cat([denorm(test_real_raw.data)[i:i + 1, :, :, :], denorm(test_fake_exp.data)[i:i + 1, :, :, :]], 3)
                    save_compare_path = os.path.join(test_compare_save_path, '{:s}_{:0>3.2f}_testRealRaw_testFakeExp.png'.format(test_name[i], self.args.pretrained_model))
                    os.makedirs(os.path.dirname(save_compare_path), exist_ok=True)
                    save_image(save_imgs_compare, save_compare_path)
            ...
            if self.args.is_test_nima:
                self.nima_result_save_path = os.path.join(self.test_result_path, 'nima_test_results')
                os.makedirs(self.nima_result_save_path, exist_ok=True)
                curr_nima = calc_nima(test_save_path, self.nima_result_save_path, self.args.pretrained_model)
                print("====== Avg. NIMA: {:>.4f} ======".format(curr_nima))

            if self.args.is_test_psnr_ssim:
                self.psnr_save_path = os.path.join(self.test_result_path, 'psnr_test_results')
                os.makedirs(self.psnr_save_path, exist_ok=True)
                curr_psnr = calc_psnr(test_save_path, self.args.test_label_dir, self.psnr_save_path, self.args.pretrained_model)
                print("====== Avg. PSNR: {:>.4f} dB ======".format(curr_psnr))

                self.ssim_save_path = os.path.join(self.test_result_path, 'ssim_test_results')
                os.makedirs(self.ssim_save_path, exist_ok=True)
                curr_ssim = calc_ssim(test_save_path, self.args.test_label_dir, self.ssim_save_path, self.args.pretrained_model)
                print("====== Avg. SSIM: {:>.4f}  ======".format(curr_ssim))
